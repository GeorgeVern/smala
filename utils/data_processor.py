...
class PawsxProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        lg = self.language if self.train_language is None else self.train_language
        lines = self._read_tsv(os.path.join(data_dir, "x-final/{}/train.tsv".format(lg)))
        examples = []
        for i, line in enumerate(lines):
            if i == 0:
                continue
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        lines = self._read_tsv(os.path.join(data_dir, "x-final/{}/dev_2k.tsv".format(self.language)))
        examples = []
        for i, line in enumerate(lines):
            if i == 0:
                continue
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        lines = self._read_tsv(os.path.join(data_dir, "x-final/{}/test_2k.tsv".format(self.language)))
        examples = []
        for i, line in enumerate(lines):
            if i == 0:
                continue
